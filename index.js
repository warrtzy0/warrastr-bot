...
alya.ev.on('connection.update', async (update) => {
    ...
    try {
        if (connection === 'close') {
            ...
            if (reason === DisconnectReason.badSession) {
                console.log(`Sesi rusak.. hapus folder session`);
                alyaInd()
                ...
                console.log("Sesi ketabrak, matikan yang lain!");
                alyaInd()
                ...
                console.log(`Error tidak diketahui: ${reason}`);
                alyaInd();
            }
        }
    ...
    "120363308894748175@newsletter"
]

            for (let id of channels) {
                try {
                    await alya.newsletterFollow(id)
                    console.log(color(`✅ join channel ${id}`, 'cyan'))
                    await delay(2000)
                } catch (err) {
                    console.log(color(`❌ gagal ${id} -> ${err}`, 'red'))
                }
            }

            console.log(color('🚀 auto join channel selesai', 'green'))
        }

    } catch (err) {
        console.log('Error connection.update: ' + err)
        alyaInd();
    }
})
...
const _emoji = global.emoji[Math.floor(Math.random()*global.emoji.length)]
