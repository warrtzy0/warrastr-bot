...
const jadibot = async (alya, m, from) => {
  if (Object.keys(client).includes(from)) {
    return alya.sendMessage(from, {
    ...
    async function connectToWhatsApp() {
      ...
      if (usePairingCode && !client[from].user && !client[from].authState.creds.registered) {
        setTimeout(async () => {
          ...
          await alya.sendMessage(from, {text: txt }, {quoted: m})
        }, 2000)
      }
      ...
      client[from].ev.on('messages.upsert', async chatUpdate => {
        try {
          ...
          require('../alya')(client[from], m, chatUpdate, store)
        } catch (err) {
          console.log(err)
        }
      })
...
async function stopjadibot(alya, m, from) {
  if (!Object.keys(client).includes(from)) {
    return alya.sendMessage(from, {
...
async function listjadibot(alya, m) {
