...
async function pagi(alya, x, data) {
    ...
    return await alya.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(alya, x, data) {
    let row = [];
    let ment = [];
...
async function malam(alya, x, data) {
...
async function skill(alya, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await alya.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, alya, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await alya.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
async function run(alya, id, data) {
    while (getWinner(id, data).status === null) {
        ...
        if (getWinner(id, data).status != null) {
            ...
            await pagi(alya, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, alya, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(alya, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, alya, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(alya, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, alya, data);
            break;
        } else {
            await skill(alya, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, alya, data);
}

async function run_vote(alya, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, alya, data);
            break;
...
async function run_malam(alya, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, alya, data);
            break;
...
async function run_pagi(alya, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, alya, data);
            break;
